...
load_orendain_ela <- function(data_dir, supp_features=TRUE) {
  ...
  mh_p_ksads <- read_csv(mh_p_ksads_path) %>%
    dplyr::select(src_subject_id, eventname, ksads_ptsd_raw_761_p,
  ...
  mh_p_fhx <- read_csv(mh_p_fhx_path) %>%
    dplyr::select(src_subject_id, eventname, famhx_4_p,
  ...
  ce_p_nsc <- read_csv(ce_p_nsc_path) %>%
    dplyr::select(src_subject_id, eventname, neighborhood3r_p,
           neighborhood2r_p)
  ...
  ce_y_fes <- read_csv(ce_y_fes_path) %>%
    dplyr::select(src_subject_id, eventname, fes_youth_q6, 
  ...
  load_supp_ela <- function() {
    ...
    # Function to read CSV file and dplyr::select the specified columns
    read_and_select <- function(file_path, var_names) {
      # Read the CSV file
      data <- read_csv(paste0(file_path, ".csv"))
      # dplyr::select the specified columns
      selected_data <- data %>% dplyr::select(all_of(var_names))
...
load_brieant_ela <- function(data_dir) {
  ...
  fhx <- read.csv(paste(data_dir, 'mental-health/mh_p_fhx.csv', sep='/')) %>%
    filter(eventname == "baseline_year_1_arm_1") %>% 
    dplyr::select(src_subject_id, famhx_ss_fath_prob_alc_p, famhx_ss_moth_prob_alc_p, famhx_ss_fath_prob_dg_p, famhx_ss_moth_prob_dg_p)
...
non_zero_data <- combined_data %>%
  dplyr::select(-src_subject_id, -eventname) %>%  # Exclude these columns from the analysis
  summarise(across(everything(), ~mean(. != 0, na.rm = TRUE))) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "prop_non_zero")
# Filter the tibble to include only those with prop_non_zero less than 0.5%
vars_to_include <- non_zero_data %>%
  filter(prop_non_zero >= 0.005) %>%
  # filter(prop_non_zero >= 0.0005) %>%
  pull(variable)
endorsed_data <- combined_data %>%
  dplyr::select(all_of(c("src_subject_id", "eventname", vars_to_include)))
...
no_covariate_data <- endorsed_data %>%
  ...
  dplyr::select(-neighborhood2r_p, -neighborhood3r_p) %>%
  dplyr::select(-eventname) # We are only taking data from baseline
