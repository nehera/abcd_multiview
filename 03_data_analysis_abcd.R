...
analyses_list <- split(analyses_df, seq(nrow(analyses_df)))
names(analyses_list) <- paste("analysis", 1:length(analyses_list), sep = "_")
...
# Define analysis and visualization function
analyze_and_visualize <- function(analysis_conditions) {
  
  # # Start with analysis_1
  # analysis_conditions <- analyses_list$analysis_1
  ...
  file_prefix <- paste(date_today, "outcome", outcome_varname, "Ncomponents", analysis_conditions$r, sep = "_")
  ...
  print("Fitting BIP...")
  abcd_result <- BIPnet::BIP(dataList, IndicVar, groupList = NULL, Method = "BIP",
                             nbrcomp = r, sample = 5000, burnin = 1000)
  models_dir <- "models"
  saveRDS(abcd_result, paste0(models_dir, "/", file_prefix, "_model_fit.rds"))
...
n_analyses <- length(analyses_list)
n_cores <- detectCores()
if (n_analyses <= n_cores) {
  n_workers <- n_analyses
} else { n_workers <- n_cores }

# Get analyses results
analyses_results <- mclapply(analyses_list, analyze_and_visualize, mc.cores = 1) # TODO allow for parallelization across n_workers
