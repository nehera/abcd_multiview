---
title: "Standard ABCD Data Preparation (5.0 Data Release)"
author: "Ellery Island, Andrea Wiglesworth, Kelly Duffy"
date: "`r Sys.Date()`"
...
data_dir <- '/Users/elleryisland/Library/CloudStorage/Box-Box/ABCD STUDY/ABCD Standard Data Preparation/' # ['path',NULL]
data_dir <- NULL
...
out_initials <- 'EI' # ['initials_string']
...
timepoint_list <- c("baseline_year_1_arm_1","1_year_follow_up_y_arm_1", "2_year_follow_up_y_arm_1") # ['baseline_year_1_arm_1','6_month_follow_up_arm_1','1_year_follow_up_y_arm_1','18_month_follow_up_arm_1','2_year_follow_up_y_arm_1','30_month_follow_up_arm_1','3_year_follow_up_y_arm_1','42_month_follow_up_arm_1']
```
...
demog <- read.csv("abcd_p_demo.csv") #Demographics (Demographic Variables), Propensity Weights
demog_y <- read.csv("mh_y_ksads_bg.csv") #Youth-reported gender and sexual orientation
puberty <- read.csv("ph_p_pds.csv") #Pubertal Stage (Demographic Variables)
study_covars <- read.csv("abcd_y_lt.csv") #Age in months, Site ID (ABCD Study Design Variables), Family ID
sib_twin <- read.csv("gen_y_pihat.csv") #Sibling related variables

#MRI standard files
mri <- read.csv("mri_y_adm_info.csv") #MRI Platform information (Neuroimaging Variables)
qc <- read.csv("/Users/elleryisland/Library/CloudStorage/Box-Box/ABCD STUDY/ABCD Study 5.0 Release/core/imaging/mri_y_qc_incl.csv") # quality score (Neuroimaging Variables), NOTE: **THIS IS IN A DIFFERENT LOCATION THAN OTHER FILES**
scan_qtns <- read.csv("mri_y_adm_qtn.csv") #Pre and post-scan emotion questionnaire (Neuroimaging Variables)

#Standard files for clinical data
cbcl <- read.csv("mh_p_cbcl.csv") # Parent child behavior checklist
bpm_y <- read.csv("mh_y_bpm.csv") # Youth brief problem monitor
ksad_p <- read.csv("mh_p_ksads_ss.csv") #Parent ksads
ksad_y <- read.csv("mh_y_ksads_ss.csv") #Youth ksads
